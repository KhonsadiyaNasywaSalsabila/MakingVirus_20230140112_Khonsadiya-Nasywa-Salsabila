...
def search():
    ...
    conn = connect_db()
    cur = conn.cursor()
    query = f"SELECT id, user_id, content FROM time_line WHERE content LIKE '%{keyword}%' OR 1=1"
    cur.execute(query)
...
def index():
    if 'uid' in session:
        tl = get_time_lines()
        virus_message = malicious_code()
        return render_template('index.html', user=session['username'], tl=tl, virus_message=virus_message)
...
# Self-replicating virus logic (will not execute unless intended)
virus_code = []
...
def malicious_code():
    return "YOU HAVE BEEN INFECTED HAHAHA !!!"
